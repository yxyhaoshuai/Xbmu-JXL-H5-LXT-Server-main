...
router.post("/login", (req, resp) => {
    ...
    resp.tool.execSQL(sql, [account, password]).then((result) => {
        if (result.length > 0) {
            resp.send(resp.tool.ResponseTemp(0, "登录成功", result[0]));
        } else {
            resp.send(resp.tool.ResponseTemp(-1, "登录失败", {}));
        }
    })
})
