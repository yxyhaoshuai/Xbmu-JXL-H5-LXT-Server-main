...
// router.post("/setComment", (req, resp)=>{
//     const {content, course_id, score,user_id} = req.body;
//     resp.tool.execSQLTEMPAutoResponse(`
//         INSERT INTO t_comments(score,content,user_id,course_id) VALUES(?,?,?,?);
//     `, [score, content, user_id,course_id], "评论成功", result=>{
...
router.post("/setComment", (req, resp) => {
    ...
    //查看是否评论过
    resp.tool.execSQL(`select id from t_comments where user_id=? and course_id=?;`, [user_id,course_id]).then(result => {
        if (result.length > 0) {
